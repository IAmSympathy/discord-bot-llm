import { Client, Events, ThreadChannel, ChannelType } from "discord.js";
...
export function registerForumThreadHandler(client: Client) {
  client.on(Events.ThreadCreate, async (thread: ThreadChannel) => {
    try {
      ...
      // Extraire les images du message initial
      const images: string[] = [];
...
${userMessage}`;
      ...
      console.log(`[ForumThread] Analyse du post de ${username}: "${userMessage.substring(0, 50)}..."`);
